...
class QueryRetailerTests(unittest.TestCase):
    ...
    def test_success_builds_site_query(self, fetch):
        ...
        with mock.patch("config.searxng_engines", return_value=[]):
    ...
    @mock.patch("search.fetch_json")
    def test_engine_override_added(self, fetch):
        fetch.return_value = {"results": []}
        with mock.patch("config.searxng_engines", return_value=["bing", "mojeek"]):
            search.query_retailer("target", "levis 501")
        args, _ = fetch.call_args
        params = args[1]
        self.assertEqual(params["engines"], "bing,mojeek")
